...
export async function createForm(title, author, age_range, googl_id, img) {
    ...
    let html1_p2 = `<input name="comment" placeholder="Comments" required>
              <button type="submit" id="sendbook">Add book</button>
            </form>`;
    let age = await getAllAgeRange();
    let select = '<select name="age_range" id="age_range" required> <option value="">-- Select an option --</option>'
    for (let a in age) {
  ...
  export async function createFormSmall(res) {
    ...
    <form id='bookForm'>
                  ...
                  <input type="hidden" name="actual" value="true">
                  <input type="hidden" name="age_range" value="${res.age_range}" required>
                  <input name="comment" placeholder="Comments" required>
                  <button type="submit" id="sendbook">Add book</button>
                </form>`;
    return html;
  }